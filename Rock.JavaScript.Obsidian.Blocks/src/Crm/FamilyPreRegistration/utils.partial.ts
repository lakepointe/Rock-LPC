...
export function convertPersonToPersonRequest(person: FamilyPreRegistrationPersonBag | null | undefined): PersonRequestBag {
    ...
    return {
        ...
        // LPC CODE
        allergy: person?.allergy || defaults.allergy,
        isSelfRelease: person?.isSelfRelease || defaults.isSelfRelease
        // END LPC CODE
    };
}
...
export function getNumberAsOrdinalString(numb: number): string {
    // LPC CODE
    if ( getLang() == 'es' ) {
        if (numb == 1) { return "primera"; }
        else if (numb == 2) { return "segunda"; }
        else if (numb == 3) { return "tercera"; }
        else if (numb == 4) { return "cuarta"; }
        else if (numb == 5) { return "quinta"; }
        else if (numb == 6) { return "sexta"; }
        else if (numb == 7) { return "séptima"; }
        else if (numb == 8) { return "octava"; }
        else if (numb == 9) { return "novena"; }
        else if (numb == 10) { return "décima"; }
        else {
            const x = numb % 10;
            const y = numb % 100;
            if (x == 1 && y != 11) {
