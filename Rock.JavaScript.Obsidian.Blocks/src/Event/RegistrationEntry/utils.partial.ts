...
export function getRegistrantBasicInfo(registrant: RegistrantBag, registrantForms: RegistrationEntryFormBag[]): RegistrantBasicInfo {
    ...
    // LPC CODE
    const mobilePhoneGuidOrEmptyString = fields.find(f => f.personFieldType === RegistrationPersonFieldType.MobilePhone && f.fieldSource === RegistrationFieldSource.PersonField)?.guid || "";
    // END LPC CODE
