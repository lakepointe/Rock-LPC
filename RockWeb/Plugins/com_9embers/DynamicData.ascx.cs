...
namespace RockWeb.Plugins.com_9embers.Reporting
{
    /// <summary>
    /// Block to display dynamic report, html, xml, or transformed xml based on a SQL query or stored procedure.
    /// </summary>
    /// <remarks>
    /// This is a copy of the core v16.4 block with following changes:
    /// - Display the Select field even if not a person report, but excel or merge document options are included
    /// - Add custom action functionality
    /// - Add option to specify numeric and currency fields
    /// - Add option to download file with formated results
    /// - Fixes issue with Grid creating entity incorrectly when UrlMask contains more parameters than just {Id} (https://github.com/SparkDevNetwork/Rock/issues/4686)
	///
    /// NOTE: This block has been renamed from 'Pillars' to '9 Embers'. If installing this block on site that already
    /// has the Pillars version, run the SQL below after adding this block to site.
    ///
    /// MAKE SURE 9 EMBERS BLOCK HAS ALREADY BEEN REGISTERED FIRST!
    ///
    /// The SQL will update all block references using old Pillars block to use the 9 Embers block. After running
    /// the SQL be sure to then delete the Pillars file from the filesystem.
    /// 
    /// </remarks>

    /*
    DECLARE @PillarsBlockType int = ( SELECT TOP 1 [Id] FROM [BlockType] WHERE [Path] = '~/Plugins/rocks_pillars/Reporting/DynamicData.ascx' )
    DECLARE @9EmbersBlockType int = ( SELECT TOP 1 [Id] FROM [BlockType] WHERE [Path] = '~/Plugins/com_9embers/Reporting/DynamicData.ascx' )

	-- Verify that both blocks exist
	IF @PillarsBlockType IS NOT NULL AND @9EmbersBlockType IS NOT NULL
	BEGIN
		-- Update blocks
		UPDATE [Block]
		SET [BlockTypeId] = @9EmbersBlockType
		WHERE [BlockTypeId] = @PillarsBlockType

		DECLARE @BlockEntityTypeId int = ( SELECT TOP 1 [Id] FROM [EntityType] WHERE [Name] = 'Rock.Model.Block' )

		-- Update Attributes
		UPDATE V 
		SET [AttributeId] = N.[Id]
		FROM [Attribute] O
		INNER JOIN [Attribute] N 
			ON N.[EntityTypeId] = @BlockEntityTypeId
			AND N.[EntityTypeQualifierColumn] = 'BlockTypeId'
			AND N.[EntityTypeQualifierValue] = CAST(@9EmbersBlockType AS VARCHAR)
			AND N.[Key] = O.[Key]
		INNER JOIN [AttributeValue] V
			ON V.[AttributeId] = O.[Id]
		WHERE O.[EntityTypeId] = @BlockEntityTypeId
		AND O.[EntityTypeQualifierColumn] = 'BlockTypeId'
		AND O.[EntityTypeQualifierValue] = CAST(@PillarsBlockType AS VARCHAR)

		-- Delete Pillars block type
		DELETE [BlockType]
		WHERE [Id] = @PillarsBlockType
	END	
    */

    [DisplayName( "Dynamic Data" )]
    [Category( "9 Embers > Reporting" )]
    ...
    [TextField( "Numeric Fields", 
		Description = "Any fields that should be displayed as a numeric value", 
		IsRequired = false,
		Category = "CustomSetting",
		Key = AttributeKey.NumericFields )]
		
    [TextField( "Currency Fields", 
		Description = "Any fields that should be displayed as a currency value", 
		IsRequired = false, 
		Category = "CustomSetting",
		Key = AttributeKey.CurrencyFields )]
    ...
    [BooleanField( "Enable Paging",
        Description = "Should paging be enabled on the grid.",
        DefaultBooleanValue = true,
        Category = "CustomSetting",
        Key = AttributeKey.EnablePaging )]
    ...
    [KeyValueListField( "Custom Actions", 
		Description = "List of custom actions and the URL that user should be redirected to when an action is selected. An EntityList will be created with the selected item before user is redirected", 
		IsRequired = false, 
		DefaultValue = "", 
        KeyPrompt = "Action",
        ValuePrompt ="Url", 
		Category = "CustomSettings",
		Key = AttributeKey.CustomActions )]
		
    [TextField( "File Name", 
		Description = "If a value is entered here, the resuts of the formatted text will be saved to a file and automatically downloaded.", 
		IsRequired = false, 
		Category = "File Download",
		Key = AttributeKey.FileName )]
		
    [TextField( "File Mime Type", 
		Description = "The mime type to use for the file being downloaded.", 
		IsRequired = false, 
		Category = "File Download",
		Key = AttributeKey.FileMimeType )]
    ...
    public partial class DynamicData : RockBlockCustomSettings
    {
        ...
        private static class AttributeKey
        {
            ...
            public const string NumericFields = "NumericFields";
            public const string CurrencyFields = "CurrencyFields";
        ...
        private Dictionary<int, string[]> _dataKeyNames;
        ...
        protected void lbSave_Click( object sender, EventArgs e )
        {
            ...
            SetAttributeValue( AttributeKey.EnablePaging, cbEnablePaging.Checked.ToString() );
            SetAttributeValue( AttributeKey.MergeFields, tbMergeFields.Text );
            SetAttributeValue( AttributeKey.NumericFields, tbNumericFields.Text );
            SetAttributeValue( AttributeKey.CurrencyFields, tbCurrencyFields.Text );
            ...
            SetAttributeValue( AttributeKey.CustomActions, kvlActions.Value );
            SaveAttributeValues();
        ...
        {
            _dataKeyNames = new Dictionary<int, string[]>();
        ...
        protected override void ShowSettings()
        {
            ...
            cbEnablePaging.Checked = GetAttributeValue( AttributeKey.EnablePaging).AsBoolean();
            tbMergeFields.Text = GetAttributeValue( AttributeKey.MergeFields );
            tbNumericFields.Text = GetAttributeValue( AttributeKey.NumericFields );
            tbCurrencyFields.Text = GetAttributeValue( AttributeKey.CurrencyFields );
            ...
            kvlActions.Value = GetAttributeValue( AttributeKey.CustomActions );
            swLavaCustomization.Checked = ceFormattedOutput.Text.IsNotNullOrWhiteSpace();

        }

        private Dictionary<string, string> GetCustomActionList()
        {
            var actionList = new Dictionary<string, string>();

            var actionsString = GetAttributeValue( AttributeKey.CustomActions );
            if ( actionsString.IsNotNullOrWhiteSpace() )
            {
                actionList = actionsString.Split( new char[] { '|' }, StringSplitOptions.RemoveEmptyEntries )
                                .Select( s => s.Split( '^' ) )
                                .ToDictionary( k => k[0], v => HttpUtility.UrlDecode( v[1] ) );
            }
            return actionList;
        }
        ...
        {
            // Don't render content if in admin mode
            ...
            var customActions = GetCustomActionList();
            ...
            else
            {
                ...
                if ( dataSetSchema != null )
                {
                    ...
                    var panelTitleLava = GetAttributeValue( AttributeKey.PanelTitle );
                    ...
                    var loadGridIntoMergeFields = formattedOutput.IsNotNullOrWhiteSpace()
                        ...
                        || panelTitleLava.IsNotNullOrWhiteSpace()
                    ...
                    if ( loadGridIntoMergeFields )
                    {
                        ...
                        var lavaTables = new List<Dictionary<string, object>>();
                        ...
                        if ( LavaService.RockLiquidIsEnabled )
                        {
                            foreach ( DataTable dataTable in dataSet.Tables )
                            {
                                ...
                                else
                                {
	                                ...
	                                mergeFields.Add( "columns", colNames );
                                }
                        ...
                        mergeFields.Add( "tables", lavaTables );
                    ...
                    if ( formattedOutput.IsNullOrWhiteSpace() )
                    {
                        ...
                        foreach ( DataTable dataTable in dataSet.Tables )
                        {
                            var tblMergeFields = new Dictionary<string, object>( mergeFields );
                            tblMergeFields.Add( "TableNumber", tableNum );
                            tableNum++;
                            ...
                            if ( GetAttributeValue( AttributeKey.WrapInPanel ).AsBoolean() )
                            {
                                ...
                                var panelTitle = GetAttributeValue( AttributeKey.PanelTitle ).ResolveMergeFields( tblMergeFields );
                            ...
                            grid.AllowPaging = GetAttributeValue( AttributeKey.EnablePaging ).AsBoolean();
                            ...
                            if ( customActions.Any() )
                            {
                                ...
                                {
                                    ID = string.Format( "ddlAction_{0}", tableId++ ),
                                    CssClass = "pull-left input-width-lg"
                                ...
                                foreach ( var kv in customActions )
                                {
                                    ddlAction.Items.Add( new ListItem( kv.Key, kv.Value ) );
                                }
                                grid.Actions.AddCustomActionControl( ddlAction );
                            }
                    ...
                    {
                        string outputText = formattedOutput.ResolveMergeFields( mergeFields, enabledLavaCommands );

                        string fileName = GetAttributeValue( "FileName" );
                        string mimeType = GetAttributeValue( "FileMimeType" );
                        if ( fileName.IsNotNullOrWhiteSpace() && mimeType.IsNotNullOrWhiteSpace() )
                        {
                            if ( IsUserAuthorized( Rock.Security.Authorization.ADMINISTRATE ) && PageParameter("ShowFile") != "True" )
                            {
                                CurrentPageReference.Parameters.AddOrReplace( "ShowFile", "True" );
                                string url = CurrentPageReference.BuildUrl();
                                CurrentPageReference.Parameters.Remove( "ShowFile" );

                                nbAlert.Text = string.Format( "If you did not have Administrate permissions on this block, this file would have been downloaded: <a href='{0}'>{1}</a>.", url, fileName );
                                nbAlert.Visible = true;
                            }
                            else
                            {
                                byte[] byteArray = System.Text.Encoding.UTF8.GetBytes( outputText );
                        ...
                        {
                            phContent.Controls.Add( new LiteralControl( outputText ) );
                        }
                    }
                }
        ...
        private void DdlAction_SelectedIndexChanged( object sender, EventArgs e )
        {
            ...
            if ( ddlAction != null && ddlAction.SelectedValue.IsNotNullOrWhiteSpace() )
            {
                var grid = ddlAction.FirstParentControlOfType<Grid>();
                if ( grid != null )
                {
                    var itemsSelected = new List<int>();
                    grid.SelectedKeys.ToList().ForEach( b => itemsSelected.Add( b.ToString().AsInteger() ) );

                    if (!itemsSelected.Any() && grid.DataKeyNames.Contains("Id") )
                    {
                        gReport_GridRebind( null, e );
                        if ( grid.DataSourceAsDataTable != null )
                        {
                            var dc = grid.DataSourceAsDataTable.Columns.OfType<DataColumn>().FirstOrDefault( a => a.ColumnName == "Id" );
                            if ( dc != null )
                            {
                                foreach( DataRowView row in grid.DataSourceAsDataTable.DefaultView )
                                {
                                    itemsSelected.Add( ( row["Id"] as int? ) ?? 0 );
                                }
                            }
                        }
                    }
                    ...
                    if ( itemsSelected.Any() )
                    {
                        ...
                        entitySet.EntityTypeId = grid.EntityTypeId;
                        ...
                        { 
                            ...
                            foreach ( var id in itemsSelected )
                            {
                            ...
                            string url = ddlAction.SelectedValue.ResolveMergeFields( mergefields, CurrentPerson );
                            url = url + ( url.Contains( "?" ) ? "&" : "?" ) + "EntitySetId=" + entitySet.Id.ToString();
        ...
        private void AddGridColumns( Grid grid, DataTable dataTable )
        {
            ...
            var numericFields = GetAttributeValue( AttributeKey.NumericFields ).SplitDelimitedValues().ToList();
            var currencyFields = GetAttributeValue( AttributeKey.CurrencyFields ).SplitDelimitedValues().ToList();
            ...
            if ( !string.IsNullOrWhiteSpace( grid.PersonIdField )
                || grid.Actions.ShowExcelExport
                || grid.Actions.ShowMergeTemplate
                || GetAttributeValue( AttributeKey.CustomActions ).IsNotNullOrWhiteSpace() )
            {
            ...
            foreach ( DataColumn dataTableColumn in dataTable.Columns )
            {
                ...
                {
                    if ( dataTableColumn.ColumnName.StartsWith( "#" ) || numericFields.Contains( dataTableColumn.ColumnName ) )
                    {
                        bf.HeaderStyle.HorizontalAlign = HorizontalAlign.Right;
                        bf.ItemStyle.HorizontalAlign = HorizontalAlign.Right;
                        if ( dataTableColumn.ColumnName.StartsWith( "#" ) )
                        {
                            splitCaseName = dataTableColumn.ColumnName.Substring( 1 ).SplitCase();
                        }
                    }
                    else if ( dataTableColumn.ColumnName.StartsWith("$") || currencyFields.Contains( dataTableColumn.ColumnName ) )
                    {
                        bf = new CurrencyField();
                        if ( dataTableColumn.ColumnName.StartsWith( "$" ) )
                        {
                            splitCaseName = dataTableColumn.ColumnName.Substring(1).SplitCase();
                        }
                    }
                    else if ( encryptedFields.Contains( dataTableColumn.ColumnName ) )
                    {
        ...
        ///
        /// </summary>
        private class DataRowDrop : LavaDataObject, ILavaDataDictionary
        {
            private readonly DataRow _dataRow;
