...
namespace org.lakepointe.Checkin.Workflow.Action.Checkin
{
    ...
    [Description( "Creates Check-Out Labels with checked out by information" )]
    [Export( typeof( ActionComponent ) )]
    [ExportMetadata( "ComponentName", "LPC Create Check-Out Labels" )]
    public class CreateCheckoutLabels : CheckInActionComponent
    {
        ...
        public override bool Execute( RockContext rockContext, WorkflowAction action, Object entity, out List<string> errorMessages )
        {
            ...
            if ( checkInState != null )
            {
                ...
                var checkedOutByPersonAliasId = checkInState.CheckIn.CheckedInByPersonAliasId;
                if ( family != null )
                {
                        ...
                        foreach ( var attendanceRec in attendanceRecs )
                        {
                            ...
                            if ( !person.Labels.Any( l => l.LabelKey.StartsWith( key ) ) )
                            {
                                ...
                                if ( groupType != null )
                                {
                                    ...
                                    Person checkedOutByPerson = null;

                                    if ( checkedOutByPersonAliasId.HasValue )
                                    {
                                        checkedOutByPerson = new PersonAliasService( rockContext ).GetPerson( checkedOutByPersonAliasId.Value );
                                    }

                                    var mostRecentAttendance = groupLocAttendance.OrderByDescending( a => a.StartDateTime ).FirstOrDefault();
                                    string securityCode = String.Empty;

                                    if ( mostRecentAttendance != null && mostRecentAttendance.AttendanceCodeId.HasValue )
                                    {
                                        securityCode = mostRecentAttendance.AttendanceCode.Code;
                                    }
                                    ...
                                    foreach ( var labelCache in groupTypeLabels.OrderBy( l => l.LabelType ).ThenBy( l => l.Order ) )
                                    {
                                        ...
                                        mergeObjects.Add( "CheckedOutByPerson", checkedOutByPerson );
                                        mergeObjects.Add( "PersonSecurityCode", securityCode );
