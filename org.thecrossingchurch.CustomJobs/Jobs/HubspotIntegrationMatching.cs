        ...
        public virtual void Execute(IJobExecutionContext context)
        {
            ...
            foreach ( int id in personIds )
            {
                if ( !hsPersonIds.Contains( id ) )
                {
                    ...
                    if ( person != null )
                    {
                        if ( hsPersonEmails.Contains( person.Email.ToStringSafe().ToLower() ) ) { continue; } // If this person's email is already in HubSpot don't include the email with this newly added person.
                        if ( hsPersonIds.Contains( person.Id ) ) { continue; } // redundant
